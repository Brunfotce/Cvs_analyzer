...
def generate_comparison_pdf(old_data, new_data, old_file_name, new_file_name, output_path=None):
    ...
    story.append(Paragraph("🏆 Top Categories Comparison", styles['Heading3']))
    ...
    top_comparison_data = [['Rank', 'Old File Category', 'Old Amount', 'New File Category', 'New Amount']]
    max_rows = max(len(old_sorted), len(new_sorted))
    
    for i in range(max_rows):
        rank = str(i + 1)
        old_cat = old_sorted[i][0] if i < len(old_sorted) else '-'
        old_val = f'{old_sorted[i][1]:,.2f}' if i < len(old_sorted) else '-'
        new_cat = new_sorted[i][0] if i < len(new_sorted) else '-'
        new_val = f'{new_sorted[i][1]:,.2f}' if i < len(new_sorted) else '-'
        
        top_comparison_data.append([rank, old_cat, old_val, new_cat, new_val])
    
    top_comparison_table = Table(top_comparison_data, colWidths=[0.4*inch, 1.8*inch, 1*inch, 1.8*inch, 1*inch])
    top_comparison_table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), colors.darkblue),
        ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
        ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
        ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
    ...
    story.append(top_comparison_table)
